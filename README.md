...
- arch/ — архитектурные заметки и регистровые карты (например Zen4: `arch/zen4/zen4_registers.md`).
...
6. Выше была лирика. А теперь по делу.  В зависимоти от Вашей IDE документация на использовнаия субагентов\subagent\agents  у всех разная. Смотрите документацию
   таких cli ide как opencode, factory droid, claudecode. Копируете агентов из папки `agent/` в нужные пути вашей cli ide + делаете изменения согласно документации. Вам надо будет указать и модель которую будете вызывать для субагентов. Можно на каждую роль отдельную модель назначить,исходя из сильных сторон. Далее копируете содержимое репозитория в рабочую область, КРОМЕ папки `agent/`. Всё готово!
7.  При указании стартового промта,советую Максимально детализировать и не жалеть токенов для описания. От этого зависит всё в реализации. От этого зависит количсетво следующих вопросов и их направленность для уточнения. заввисят сквозные правила которые протянутся на всех этапах на каждого агента.
...
- arch/ — architecture notes and register maps (e.g. Zen4: `arch/zen4/zen4_registers.md`).

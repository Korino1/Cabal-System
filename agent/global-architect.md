...
3) Явное указание зависимостей задач, входов/выходов и критериев готовности для каждой задачи.
...
2) Сессия GA-2 (новый контекст): читаешь записи GA-1 из `.memory` + файлы проекта, делишь блоки на методы. **Строго: 1 блок = 1 сессия одного агента** (один агент не обрабатывает два блока одновременно; разные агенты могут параллельно обрабатывать разные блоки). Запись — в `.memory/LOGIC_PROTOCOL.md` (раздел GA-2), затем закрываешь сессию.
...
6) После завершения GA-5 передать результат Оркестратору. Назначение тестов и контроль их создания — обязанность Оркестратора (orchestrator.md:82) после реализации кода.

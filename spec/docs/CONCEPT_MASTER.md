...
- Дополнительные правила `YOLO` являются дополнением к существующим правилам секций 6.1..6.9 и не отменяют их.
...
- Перед началом каждой новой сессии/итерации/назначения любой агент и субагент обязан перечитать полный актуальный перечень сквозных правил (`6.1..6.9`) в `spec/docs/CONCEPT_MASTER.md`.
- Если сквозные правила расширены (в том числе через `YOLO`), агент/субагент обязан ознакомиться со всем перечнем правил с учётом дополнений до старта работы.
- Постановка задачи считается некорректной, если исполнитель не получил актуальные сквозные правила перед началом работы.
- Для режима `YOLO` Оркестратор обязан в каждой CONSULT-постановке передавать исполнителю полный актуальный набор сквозных правил (база + дополнения).
- Любое действие, выполненное без актуального ознакомления со сквозными правилами, считается нарушением протокола 3.1 и оформляется как `CONSULT/DEFECT`.
...
- Если выбран `YOLO`, какие дополнительные сквозные правила нужно добавить к секциям 6.1..6.9 до активации режима?
- Как подтверждается, что каждый агент/субагент перед каждой задачей перечитал полный актуальный список сквозных правил?

...
7) Критический запрос к Математику после получения proof (обязательное):
   - После получения от `mathematician` математической проверки (леммы/доказательства) инициировать CONSULT для назначения Оркестратором отдельной сессии математика-критика.
   - Оркестратор назначает `mathematician` в режиме критика и контролирует результат.
   - Математик-критик обязан обозначить:
     - слабые/неучтённые места;
     - уязвимые параметры и условия применимости;
     - риски некорректности и зоны неопределённости.
   - Математик-критик предоставляет 2 варианта решения для Пользователя (с краткими компромиссами).
8) Работа с ответом Пользователя (обязательное):
   - Если Пользователь предлагает свой метод: инициировать CONSULT для назначения Оркестратором отдельной валидации метода у Математика-критика (корректность/ограничения/риски).
   - Если Пользователь не предлагает метод и просит поиск решения: Оркестратор назначает Математику поиск решения через 5-язычный цикл (RU/EN/ZH/DE/FR) с синтезом.
9) Синхронизация канона после выбора Пользователя (обязательное):
   ...
   - **Лимит повторных циклов:** допускается не более 3 повторных циклов C-0.2..C-0.6. После 3 циклов параметры фиксируются как «замороженные» (freeze); фаза C-0 завершается, переход в GA-1 обязателен.

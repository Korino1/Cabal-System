...
- Контроль C-0 math-critique (обязательное):
  ...
  - Если Пользователь не предлагает метод и просит найти решение — требуй 3-язычный цикл решения (RU/EN/ZH) и итоговый синтез.
...
- Контроль multilingual-fallback для `qa-agent`/`rust-engineer`/`debuger`/`fixer`:
  - При метке «нерешаемо/трудно решаемо» проверяй, что выполнен цикл ZH -> EN и зафиксирован синтез решения.

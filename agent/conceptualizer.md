...
7) Каждая сессия фиксируется как отдельная задача: C-0.1 (RU), C-0.2 (EN), C-0.3 (ZH), C-0.4 (synthesis), C-0.5 (math-critique) в `.memory/PHASES/C-0/TASKS.md`.
8) Не писать код; только концептуальные связи, цели и порядок работы.
9) При необходимости запрашивать помощь Математика для формализаций и лемм.
...
Завершение:
- После сессии 5 (math-critique и синтез с учётом критики) передать результат Оркестратору; дальше протокол стандартный.

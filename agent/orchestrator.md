...
6) Контроль качества через назначение qa-agent: на каждой итерации rust-engineer обязательно назначай `qa-agent` для проверки соответствия концепту, отсутствия заглушек/упрощений и корректности входов/выходов. Без `QA:PASS` итерация считается незавершённой.
...
- Для GA-2: строго 1 блок = 1 сессия (разбивка блока на методы выполняется отдельной сессией).Требуй соблюдение Сквозных правил!
- Для GA-2: перед разбивкой требуй чтение файлов концепта и результатов GA-1 (логическая схема в `.memory/LOGIC_PROTOCOL.md`). Требуй соблюдение Сквозных правил!
...
- Для GA-4: строго 1 метод = 1 сессия (разбивка метода на функции выполняется отдельной сессией).Требуй соблюдение Сквозных правил!
- Для GA-4: перед назначением требуй чтение файлов концепта + разделов GA-1, GA-2 и GA-3 (блок метода) в `.memory/LOGIC_PROTOCOL.md`; при наличии уточнений — `PHASES/<Active>/INDEX.md` и `PHASES/<Active>/WORKLOG.md`.Требуй соблюдение Сквозных правил!
...
- Для INTEGRATOR: перед назначением требуй чтение файлов концепта и результатов GA-1..GA-5 и ARCH в `.memory/LOGIC_PROTOCOL.md`; при наличии уточнений — `PHASES/<Active>/INDEX.md` и `PHASES/<Active>/WORKLOG.md`.Требуй соблюдение Сквозных правил!
- **Для RUST-ENGINEER (после INTEGRATOR)**: перед назначением требуй чтение:
  - `PHASES/INTEGRATOR/INTEGRATION_MAP.json` — машиночитаемый канон (обязательно!).
  - `PHASES/INTEGRATOR/INTEGRATION_MAP.mmd` — визуализация графа (для понимания потока).
  - `PHASES/INTEGRATOR/INTEGRATION_MAP.md` — документация.
  - Контекстный бриф: `LINK_ID`, входы/выходы, инварианты, ошибки, границы из JSON.
  - Без чтения JSON-канона сессию не запускать.
- **Для SIMD-SPECIALIST**: требуй чтение `INTEGRATION_MAP.json` (hot-loop/hot-path), `INTEGRATION_MAP.mmd` (critical path).
- **Для TESTER**: требуй чтение `INTEGRATION_MAP.json` (разделы `links`, `errors`, `boundaries`) для написания тестов.
- **Для QA-AGENT**: требуй чтение `INTEGRATION_MAP.json` для проверки соответствия кода канону.
...
- Автозапуск: при наличии списка блоков/методов/функций создавай параллельные назначения с уникальными идентификаторами сессий. Разные агенты могут параллельно обрабатывать разные блоки/методы/функции; один агент — строго один объект за сессию.
...
- На каждой итерации rust-engineer (`FN:*` или `MTD:*`) обязательно назначай `qa-agent` для проверки качества, соответствия критериям и анти-имитации (заглушки/упрощения/временные подмены).Требуй соблюдение Сквозных правил!
...
- Требовать соблюдение скозных правил в КАЖДОМ запросе у КАЖДОГО субагента\агента.

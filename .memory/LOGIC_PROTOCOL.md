...
- Переход в GA-1 разрешён только после завершения последнего (актуального) цикла C-0.2..C-0.6.
...
### 0.4) Обязательное ознакомление со сквозными правилами перед каждой сессией (обязательное)
- Перед началом каждой новой задачи/итерации/сессии любой agent/subagent обязан ознакомиться с полным актуальным перечнем сквозных правил из `spec/docs/CONCEPT_MASTER.md` (раздел 6).
- Если список сквозных правил был расширен, работа не начинается до ознакомления исполнителя с полной версией правил (базовые + дополнения).
- Для режима `YOLO` Оркестратор обязан включать в каждую CONSULT-постановку полный актуальный набор сквозных правил для назначаемого исполнителя.
- Нарушение этого требования считается дефектом протокола и оформляется как `CONSULT/DEFECT` с остановкой текущей итерации до устранения.
...
### 4.1) Мультиязычный fallback при труднорешаемых low-level проблемах (rust-engineer / simd-specialist / debuger / fixer; для QA при проверке этих ролей)
...
- Перед стартом каждой своей задачи agent\subagent обязан подтвердить ознакомление с полным актуальным списком сквозных правил (включая расширения).

...
- Во время выполнения любой задачи регулярно оценивай, не появились ли неоднозначности или развилки, способные усложнить проект; при их обнаружении приостанови реализацию и инициируй CONSULT по активному маршруту.
- При любом появлении альтернативных подходов, спорных вопросов или необходимости усложнить архитектуру остановись и запроси CONSULT по активному маршруту до продолжения работы.
...
- Каждую нетривиальную развилку (варианты архитектуры, изменение контрактов, выбор зависимостей, изменение SLA) предваряй отдельной задачей `CONSULT — …` и маршрутизируй её по активному режиму (`USER_TRACKING`/`YOLO`).
...
- Маршрут CONSULT задаётся в `spec/docs/CONCEPT_MASTER.md` (раздел 6.7): `USER_TRACKING` → Пользователь, `YOLO` → Оркестратор.
...
- После math-critique и выбора Пользователем варианта/параметров обновления сначала фиксировать в `spec/docs/CONCEPT_MATH_PROOF.md` и `spec/docs/CONCEPT_MASTER.md` (C-0); перенос этих изменений в GA-5 запрещён.
- Если после C-0.6 Пользователь меняет вариант/параметры, повторно открыть и выполнить цикл C-0.2..C-0.6; переход в GA-фазы до закрытия нового цикла запрещён.
- Перед первым запуском GA-1 обязательно запросить у Пользователя режим маршрутизации CONSULT (`USER_TRACKING` или `YOLO`) и зафиксировать его в `spec/docs/CONCEPT_MASTER.md` (6.7).
- При выборе `YOLO` до активации режима обязательно запросить у Пользователя дополнительные сквозные правила и зафиксировать их как дополнение к текущим правилам; только после этого запускать GA-1.
